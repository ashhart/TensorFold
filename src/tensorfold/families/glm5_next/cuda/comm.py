"""Cross-rank all-gather for GLM-5.3-Flash's tensor parallelism: NCCL on the caller's CUDA stream.

Every reduction in the engine is an all-gather of fp32 partials followed by a fixed rank-order sum, so both
ranks hold identical bits. The communicator is NCCL through ctypes, so the all-gathers take raw pointers on the
current stream and are captured in the decode steps' CUDA graphs.
"""
...
def _library() -> ctypes.CDLL:
    ...
    candidates += glob.glob("/usr/lib/*/libnccl.so.2")
    candidates += glob.glob("/usr/local/lib/python3*/dist-packages/nvidia/nccl/lib/libnccl.so.2")
    ...
    for path in candidates:
        if path:
            try:
                ...
                continue
    raise RuntimeError("libnccl not found (set TF_NCCL_LIB to its path)")


class NCCL:
    """Two ranks on two machines. The unique id travels through a TCPStore on rank 0's address and port."""
    ...
    def __init__(self, rank: int, world: int, master: str, port: int) -> None:
        ...
        self.store = TCPStore(master, port, world, rank == 0, timeout=timedelta(seconds=900))
        uid = _UniqueId()
        if rank == 0:
            self._check(lib.ncclGetUniqueId(ctypes.byref(uid)))
            self.store.set("tf_glm_nccl_uid", bytes(uid.internal))
        else:
            raw = self.store.get("tf_glm_nccl_uid")
            ctypes.memmove(ctypes.addressof(uid), raw, 128)
        self.comm = ctypes.c_void_p()
        torch.cuda.current_device()
        self._check(lib.ncclCommInitRank(ctypes.byref(self.comm), world, uid, rank))
